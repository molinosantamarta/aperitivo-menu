(() => {
  ...
  var serviceCallLightboxDialog = document.querySelector("#serviceCallLightboxDialog");
  ...
  function applySheetPayloadToMenu(baseMenu, payload) {
    const withSections = applySheetSectionsToMenu(baseMenu, Array.isArray(payload == null ? void 0 : payload.sections) ? payload.sections : []);
  ...
  function applySheetSectionsToMenu(baseMenu, sectionRows) {
    ...
    const rowLookup = new Map(
      sectionRows.filter((row) => row && row.section_id).map((row) => [row.section_id, row])
    );
    nextMenu.sections = nextMenu.sections.filter((section) => {
      ...
      return section;
    }).sort((left, right) => getSheetPosition(left) - getSheetPosition(right)).map((section) => {
  ...
  function buildSheetPhotoPanelVisual(row, imageUrl) {
    ...
    const isMilkshake = ((row == null ? void 0 : row.id) || "") === "milkshake";
    return {
      type: "photo-panel",
      asset: imageUrl,
  ...
  function renderGeneratedCartSummary() {
    ...
    (_c = actions.querySelector('[data-generated-action="call-waiter"]')) == null ? void 0 : _c.addEventListener("click", (event) => {
