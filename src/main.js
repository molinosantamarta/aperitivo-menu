...
const LOADER_MIN_DURATION = 8000;
...
const SLOW_FONT_LOADER_MESSAGES = [
...
const PROMO_AGRI_VIDEOS = [
  {
    ...
    eyebrow: "BBQ sotto le Stelle 2025",
...
const BLOCKING_REQUIRED_FONT_PLANS = REQUIRED_FONT_PLANS.filter((plan) => plan?.blocking === true);
const NON_BLOCKING_REQUIRED_FONT_PLANS = REQUIRED_FONT_PLANS.filter((plan) => plan?.blocking !== true);
...
function loadClarityScript() {
  ...
  script.onerror = () => {
    // Ignore analytics failures to keep Safari and the app shell responsive.
  };
...
function shouldEnableFontGateDebug() {
  try {
    ...
    const storedValue = window.localStorage?.getItem(FONT_GATE_DEBUG_STORAGE_KEY);
...
function logFontGate(message, details) {
  ...
  const prefix = `${FONT_GATE_LOG_PREFIX} +${getFontGateElapsedMs()}ms ${message}`;
...
async function init() {
  ...
  const fontsReadyPromise = waitForCriticalFontGate()
    .then(() => {
      syncDisplayFontSpacingFromMetrics();
...
function showBootstrapFailureState(error) {
  ...
  const isFontGateFailure = error?.bootPhase === "fonts";
  setAppInteractivityLocked(true);
  setAppReadyState(false, isFontGateFailure ? "blocked-timeout" : "blocked-error", error?.bootPhase || "bootstrap");
  logFontGate("bootstrap:blocked", {
    phase: error?.bootPhase || "unknown",
    error: error?.message || String(error),
  });
  ...
  if (title) {
    title.textContent =
      isFontGateFailure
        ? "Stiamo ancora preparando il menu."
  ...
  if (note) {
    note.textContent =
      isFontGateFailure
        ? "Aspetto ancora i font critici per mostrare il layout corretto e stabile. Il menu resta volutamente bloccato finché non sono pronti. Puoi riprovare."
...
function buildSheetPhotoPanelVisual(row, imageUrl) {
  ...
  return {
    type: "photo-panel",
    asset: imageUrl,
    position: row.visual_position || row.image_position || (isMilkshake ? "center 62%" : "center center"),
    size: row.visual_size || row.image_size || (isMilkshake ? "82% auto" : "cover"),
...
function getSheetRowName(row) {
  return getFirstSheetValue(row?.name, row?.nome, row?.nome_attuale_solo_riferimento);
}

function getSheetRowDescription(row) {
  return getFirstSheetValue(row?.description, row?.descrizione);
}

function getSheetRowCategory(row) {
  return getFirstSheetValue(row?.category, row?.categoria);
}

function getSheetRowSectionId(row) {
  return getFirstSheetValue(row?.section_id, row?.sezione);
}
...
async function waitForCriticalFontGate() {
  if (!("fonts" in document) || !document.fonts?.load || !document.fonts?.check) {
...
async function waitForLoaderFonts() {
  if (!("fonts" in document) || !document.fonts?.load || !document.fonts?.check || !LOADER_FONT_PLANS.length) {
...
function getItemCardStyle(item, availabilityState) {
  ...
  return [
    ...
    `--coming-soon-mid: ${mid}`,
    `--coming-soon-end: ${end}`,
    `--coming-soon-text-color: ${comingSoonTextColor}`,
    `--coming-soon-text-shadow: ${
      usesDarkText ? "0 1px 0 rgba(255, 255, 255, 0.2)" : "0 1px 0 rgba(76, 32, 18, 0.22)"
    }`,
  ].join("; ");
}
    ...
    optionButton.innerHTML = displayLabel
      ? `
          <span class="option-copy">
            ...
            ${
              optionSubtitle
                ? `<span class="option-subtitle${optionSubtitleClass ? ` ${optionSubtitleClass}` : ""}">${optionSubtitle}</span>`
                : ""
            }
          </span>
...
function renderMultiOptionQuantityGroup(item, label = "") {
  ...
  item.options.forEach((option) => {
    ...
    const showPrice = !shouldHideDetailOptionPrices(item) && Number.isFinite(option?.price);
    const quantityBadgeMarkup =
      !usesEmbeddedSelector && optionQuantity > 0
        ? `
    ...
    const inlineSelectorMarkup = usesEmbeddedSelector
      ? `
          <div class="option-qty-inline" aria-label="Quantità ${safeOptionLabel}">
            <button
              class="qty-btn option-qty-inline__btn"
              type="button"
              data-option-qty-action="decrease"
              aria-label="Riduci ${safeOptionLabel}"
              ${optionQuantity <= 0 ? "disabled" : ""}
            >
              −
            </button>
            <span class="option-qty-inline__value" aria-live="polite">${optionQuantity}</span>
            <button
              class="qty-btn option-qty-inline__btn"
              type="button"
              data-option-qty-action="increase"
              aria-label="Aggiungi ${safeOptionLabel}"
            >
              +
            </button>
          </div>
    ...
    optionCard.className = `option-qty-card${toneClass ? ` ${toneClass}` : ""}${
      ...
      usesEmbeddedSelector ? " option-qty-card--with-inline-selector" : ""
    }${
    ...
    optionCard.setAttribute(
      "aria-label",
      usesEmbeddedSelector
        ? `${optionLabel}, quantità ${optionQuantity}. Tocca la card o il più per aggiungere, usa il meno per ridurre.`
        : optionQuantity > 0
    ...
    optionCard.innerHTML = `
      ...
      <div class="option-copy">
        ...
        ${
          optionSubtitle
            ? `<span class="option-subtitle${optionSubtitleClass ? ` ${optionSubtitleClass}` : ""}">${escapeHtml(optionSubtitle)}</span>`
            : ""
        }
        ${showPrice ? `<span class="option-price">${formatPrice(option.price)}</span>` : ""}
      </div>
      ${inlineSelectorMarkup}
      ${quantityBadgeMarkup}
      ${removeButtonMarkup}
    `;
...
function getMultiOptionSelectionSummaryLabel(item, totalQuantity) {
  ...
  if (item.id === "acqua") {
    return `${totalQuantity} ${pluralize(totalQuantity, "bottiglia", "bottiglie")}`;
  }

  if (item.id === "bibite") {
    return `${totalQuantity} ${pluralize(totalQuantity, "bibita", "bibite")}`;
  }
...
function renderQuantityControl() {
  ...
  detailQuantity.innerHTML = `
    <div class="detail-quantity__pill" aria-label="Seleziona quantita" ${quantityDisabled ? 'aria-disabled="true"' : ""}>
      <button
        class="qty-btn detail-quantity__btn"
        type="button"
        aria-label="Riduci quantita"
        ${state.selectedQuantity <= quantityFloor || quantityDisabled ? "disabled" : ""}
      >
        −
      </button>
...
function updatePhotoPanelSelectionState(item) {
  ...
  if (activeImage) {
    ...
    activeImage.style.setProperty("--photo-panel-selection-swap-offset", `${direction * -swapDistance}px`);
  ...
  const direction =
    Number.isInteger(activeSelectionIndex) && activeSelectionIndex !== nextSelectionIndex
      ...
      : 1;
  const swapDistance = Math.max(180, Math.min(window.innerWidth * 0.52, 260));
  incomingImage.style.setProperty("--photo-panel-selection-swap-offset", `${direction * swapDistance}px`);
...
function getSelectedSelectionLabels(item) {
  return getSelectionGroups(item)
    .map((group) => {
      const selectedIndex = getSelectedSelectionIndex(group);
      return selectedIndex >= 0 ? group.options[selectedIndex]?.label || "" : "";
    })
    .filter(Boolean);
}
...
function getDetailPreviewSlides(item) {
  ...
  const baseSlides = hasDetailGallery(item)
    ? item.detailGallery.filter((visual) => visual && visual.type !== "none")
    : detailVisual && detailVisual.type !== "none"
      ? [detailVisual]
      : [];
          ...
          (canItem, index) => `
            <img
              ...
              style="
                ...
                --can-shift-y: ${getCanClusterItemValue(canItem, "shiftY", context, "0px")};
                --can-z: ${
                ...
                --can-scale: ${
                  context === "detail"
                    ? activeCanIndex >= 0 && index === activeCanIndex
                      ? detailSelectedScale
                      : detailDefaultScale
                    : getCanClusterItemValue(canItem, "scale", context, "0.74")
                };
              "
            />
          `
...
function resolveMenuAssetPath(assetName) {
  ...
  return buildVersionedPath(`./menu-assets/items/${normalized}`);
}
