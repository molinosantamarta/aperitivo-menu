...
async function waitForRequiredFonts() {
  ...
  await Promise.all([
    ...CRITICAL_FONT_DESCRIPTORS.map((descriptor) =>
      waitForFontLoad(descriptor, {
        strict: true,
  ...
  while (true) {
    ...
    try {
      ...
      if (strict && performance.now() >= deadline) {
        throw new Error(`Timeout nel caricamento del font critico: ${descriptor}`);
      }
    }
...
function initFormatCarousel() {
  ...
  formatCarouselDots.innerHTML = slides
    .map(
      (_, index) => `
        <button
          class="format-carousel__dot"
          type="button"
          aria-label="Vai al format ${index + 1}"
  ...
  const syncCarousel = () => {
    formatCarouselTrack.style.transform = `translateX(-${activeIndex * 100}%)`;
